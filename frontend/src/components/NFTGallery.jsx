...
import { ExternalLink, Loader2, Hexagon } from 'lucide-react';
import { formatPublicKey } from '../utils/validation';

const NFTGallery = ({ fetchedAssets = [], isLoading = false }) => {
  if (fetchedAssets.length === 0 && !isLoading) return null;

  return (
    <div className="mt-8 w-full max-w-6xl mx-auto flex flex-col gap-12 pb-12">
      <div className="pt-8 border-t border-white/10 mt-4">
        ...
        {!isLoading && fetchedAssets.length === 0 ? (
          <div className="text-gray-400 text-base bg-white/5 p-6 rounded-xl border border-white/10 text-center font-medium">
            No NFTs yet
          </div>
        ) : (
